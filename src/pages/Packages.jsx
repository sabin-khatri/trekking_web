...
// Consistent easing + viewport settings used across the page for a snappier, uniform feel
      ...
      <div className="mt-10 sm:mt-12 flex justify-center gap-2 sm:gap-3 flex-wrap">
        {currentPage > 1 && (
          <motion.button
            ...
            className="px-4 py-2 sm:px-5 sm:py-3 rounded-lg border border-gray-300 hover:bg-green-600 hover:text-white transition-colors duration-300 text-lg"
          >
            <FaChevronLeft />
          </motion.button>
        )}
        ...
        {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
          <motion.button
            ...
            className={`px-4 py-2 sm:px-5 sm:py-3 rounded-lg border transition-colors duration-300 text-base sm:text-lg ${
              page === currentPage
                ? 'bg-green-600 text-white border-transparent shadow-md'
                : 'bg-white text-slate-700 border-gray-300 hover:bg-green-600 hover:text-white'
            }`}
          >
            {page}
          </motion.button>
  ...
  return (
    <div className="min-h-screen bg-slate-50 transition-colors duration-300">
          ...
          <div className="mt-8 flex flex-col gap-4 max-w-4xl mx-auto">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-3.5 rounded-2xl border border-slate-200 bg-white/80 backdrop-blur-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent text-base transition-shadow shadow-sm hover:shadow-md"
              />
            </div>
            ...
            <div className="flex gap-3">
              <select
                ...
                className="flex-1 min-w-0 px-4 py-3.5 rounded-2xl border border-slate-200 bg-white/80 backdrop-blur-sm text-slate-800 focus:ring-2 focus:ring-emerald-500 text-sm sm:text-base transition-shadow shadow-sm hover:shadow-md outline-none"
              >
              ...
              <div className="flex gap-2 bg-slate-100/80 backdrop-blur-sm p-1.5 rounded-2xl shrink-0">
                <button
                  onClick={() => setViewMode('grid')}
                  aria-label="Grid view"
                  className={`p-3 rounded-xl transition-all duration-200 ${viewMode === 'grid' ? 'bg-white shadow-md text-emerald-600 scale-105' : 'text-slate-500 hover:text-emerald-500'}`}
                >
                  <FaThLarge />
                </button>
                <button
                  ...
                  className={`p-3 rounded-xl transition-all duration-200 ${viewMode === 'list' ? 'bg-white shadow-md text-emerald-600 scale-105' : 'text-slate-500 hover:text-emerald-500'}`}
                >
                  <FaList />
                </button>
              </div>
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-3 sm:gap-4">
            {['all', 'moderate', 'challenging'].map((filter) => (
              <motion.button
                ...
                className={`px-7 py-3.5 rounded-full font-medium text-base transition-colors duration-300 ${
                  currentFilter === filter
                    ? 'bg-gradient-to-r from-emerald-600 to-green-600 text-white shadow-lg shadow-emerald-600/30 border-transparent'
              ...
              {paginatedTreks.length === 0 ? (
                ...
                paginatedTreks.map((trek, index) => (
                  <motion.div
                    ...
                    whileHover={{ y: -6 }}
                    key={trek.name}
                    className={`relative bg-white rounded-3xl shadow-lg shadow-slate-200/60 overflow-hidden hover:shadow-2xl hover:shadow-emerald-900/10 transition-shadow duration-300 group border border-slate-100 ${viewMode === 'list' ? 'flex flex-col sm:flex-row' : ''}`}
                  >
                    <div className={`relative overflow-hidden shrink-0 ${viewMode === 'list' ? 'sm:w-2/5' : ''}`}>
                      ...
                      {trek.featured && (
                        <span className="absolute top-4 left-4 bg-amber-400 text-amber-950 text-xs font-bold px-3 py-1 rounded-full shadow-md flex items-center gap-1">
                          ★ Featured
                        </span>
                      )}

                      <span className="absolute bottom-3 left-4 inline-flex items-center gap-1.5 text-white text-sm font-medium">
                      ...
                      <motion.button
                        ...
                        className="absolute top-4 right-4 z-10 w-10 h-10 rounded-full bg-white/90 backdrop-blur-sm flex items-center justify-center text-slate-700 hover:text-red-500 shadow-md transition-colors cursor-pointer"
                    ...
                    <div className={`p-6 flex flex-col ${viewMode === 'list' ? 'sm:w-3/5 justify-center' : ''}`}>
                      <h3 className="text-xl sm:text-2xl font-bold text-slate-800 leading-snug">{trek.name}</h3>

                      {/* Chip row: location + category */}
                      <div className="mt-3 flex flex-wrap items-center gap-2">
                        <span className="inline-flex items-center gap-1.5 text-slate-600 text-xs sm:text-sm bg-slate-100 px-3 py-1 rounded-full">
                          <FaMapMarkerAlt className="text-green-600" />
                          {trek.location}
                        </span>
                        <span className={`inline-flex items-center gap-1.5 text-xs sm:text-sm px-3 py-1 rounded-full border font-medium capitalize ${categoryStyles[trek.category] || 'bg-slate-100 text-slate-700 border-slate-200'}`}>
                      ...
                      <div className="mt-5 flex items-end justify-between flex-wrap gap-4 pt-4 border-t border-slate-100">
                        <div>
                        ...
                        <div className="flex gap-2.5">
                          <motion.button
                            ...
                            className="bg-gradient-to-r from-emerald-600 to-green-600 text-white px-4 sm:px-5 py-2.5 rounded-xl font-medium flex items-center gap-2 text-sm sm:text-base shadow-lg shadow-emerald-600/30"
                          >
                          ...
                          <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} transition={{ type: 'spring', stiffness: 400, damping: 18 }}>
                            <Link
                              to={`/packages/${trek.id}`}
                              className="border border-emerald-600 text-emerald-600 hover:bg-emerald-50 px-4 sm:px-5 py-2.5 rounded-xl font-medium transition-colors flex items-center gap-2 text-sm sm:text-base"
                            >
          ...
          <div className="mt-12 md:mt-16 grid sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {[
              ...
              <motion.div
                ...
                whileHover={{ y: -6 }}
                className="text-center p-6 md:p-8 rounded-2xl bg-white shadow-lg transition-shadow duration-300 hover:shadow-2xl border border-transparent"
              >
          ...
          >
            ...
            >
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => setShowCartModal(false)} className="text-3xl leading-none text-slate-500 hover:text-slate-800 transition-colors">
                  ×
                </button>
              </div>
                  ...
                  <div className="flex justify-between items-center flex-wrap gap-4">
                    ...
                    <div className="text-right">
                      ...
                      <button
                        ...
                        className="text-red-600 hover:text-red-800 text-sm mt-2 flex items-center gap-1.5 ml-auto transition-colors"
                      >
              ...
              <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mt-6">
                ...
                <div className="flex gap-4 w-full sm:w-auto">
                  {cart && (
                    <button
                      onClick={clearCart}
                      className="flex-1 px-6 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors text-base sm:text-lg"
                    >
                      Clear Cart
                    </button>
                  )}
                  <button
                    onClick={() => setShowCartModal(false)}
                    className="flex-1 px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors text-base sm:text-lg"
                  >
                    Continue
                  </button>
                </div>
      ...
      <motion.button
        ...
        className="fixed bottom-6 right-6 sm:bottom-8 sm:right-8 bg-gradient-to-r from-emerald-600 to-green-600 text-white p-5 sm:p-6 rounded-full shadow-2xl hover:shadow-emerald-600/50 transition-shadow z-40"
      ...
      >
        ...
        {cart && (
          <motion.span
            ...
            className="absolute -top-2 -right-2 bg-red-500 text-white text-xs sm:text-sm font-bold w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center rounded-full shadow-md"
          >
            1
          </motion.span>
        )}
